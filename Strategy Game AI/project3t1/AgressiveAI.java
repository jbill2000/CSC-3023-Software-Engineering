...
public class AgressiveAI extends AI
{
   ...
   {   
      ...
      {
         if(enemiesInRange.get(i).getHP() < enemiesInRange.get(index).getHP())
         {
            index = i;
         }
      }
      ...
      {
         m = new Move(theUnit,new Vector2(possibleSpots.get(selected).getX(),possibleSpots.get(selected).getY()),true,enemiesInRange.get(index));
      }
      else
      {
