...
public class MackenzieAi extends AI
{
   public MackenzieAi()
   {
      name = "Mackenzie";
   ...
   public void ratePossiblePlacesToMoveForAdvancement( Unit theUnit,ArrayList<MapTile> possibleSpots)
   {
      ...
      for (MapTile currentPossibleSpot : possibleSpots)
      {
         ...
         if (enemiesInRangeOfCurrentAllyAttack.size() == 0 && enemiesInRangeToAttackAfterEnemyMove.size() > 0)
         {
            currentPossibleSpot.setGoodScore(currentPossibleSpot.getGoodScore() - 5);
         }
         ...
         if (enemiesInRangeOfCurrentAllyAttack.size() == 0 && enemiesInRangeToAttackAfterEnemyMove.size() > 0)
         {
            //Not sure if best way to do this, but encourages unit to stay if theres one enemy attacking who won't kill them
            if(enemiesInRangeToAttackAfterEnemyMove.size() == 1 && theUnit.getHP() > possibleDamageTaken )
            {
               currentPossibleSpot.setGoodScore(currentPossibleSpot.getGoodScore() + 5);
            }
            //else it does what it did before which is lower score if unit will be killed or attacked by 2 of more units
            else
            {
               currentPossibleSpot.setGoodScore(currentPossibleSpot.getGoodScore() - 5);
            }
