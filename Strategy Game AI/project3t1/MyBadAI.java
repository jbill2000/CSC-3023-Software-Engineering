...
public class MyBadAI extends AI
{
   ...
   public void ratePossiblePlacesToMoveForAdvancement( Unit theUnit,ArrayList<MapTile> possibleSpots)
   {
      ...
      Map.Iterator spots = Map.getMap().generateIteratorWithinMoveRange(theUnit.getX(),theUnit.getY(),theUnit.getSpeed(),theUnit);
      while(spots.hasNext())
      {
      ...
      spots = Map.getMap().generateIteratorAllTiles();
      while(spots.hasNext())
      {
         MapTile tile = spots.next();
         //do something here.
